...
interface TerminalData {
  ...
  /**
   * Name already generated by a paid provider call but not yet applied because
   * focus could not be held. Reused on the next auto-rename attempt so we do
   * not re-bill generateName on every shell command during focus skips.
   */
...
export class TerminalTracker {
  ...
  /**
   * Bumped whenever provider-related settings change. In-flight rename
   * attempts capture the generation at start and must not re-apply cooldown
   * if the user already fixed configuration while the request was pending.
   */
  private providerConfigGeneration = 0;
  ...
  private clearAutoRenameBlocks() {
    for (const data of this.terminalDataMap.values()) {
      ...
      // Drop cached names too — language/provider changes may invalidate them.
      data.pendingName = undefined;
    }
  }
  ...
  private handleCommandExecution(event: vscode.TerminalShellExecutionEndEvent) {
    ...
    if (
      ...
      // Capture config generation so a stale in-flight failure cannot re-block
      // after the user already cleared cooldown via settings changes.
      const configGenerationAtStart = this.providerConfigGeneration;
      void Promise.resolve(this.onCommandThresholdReached(terminal, commandsSnapshot))
        .then((result) => {
          ...
          if (result === true || result === 'renamed') {
            ...
            result === 'failed' &&
            configGenerationAtStart === this.providerConfigGeneration
          ) {
        ...
        .catch(() => {
          ...
          // attempts on every subsequent shell command — unless config already
          // changed while this attempt was in flight.
          if (configGenerationAtStart === this.providerConfigGeneration) {
            data!.autoRenameBlockedUntil =
              Date.now() + AUTO_RENAME_FAILURE_COOLDOWN_MS;
          }
        })
  ...
  /**
   * Name retained after a focus-skip so the next attempt can skip generateName.
   */
  getPendingName(terminal: vscode.Terminal): string | undefined {
    return this.terminalDataMap.get(terminal)?.pendingName;
  }
  ...
  clearPendingName(terminal: vscode.Terminal) {
    const data = this.terminalDataMap.get(terminal);
    if (data) {
      data.pendingName = undefined;
    }
